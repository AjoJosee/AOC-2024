#plaN is to find area by counting and perimeter by enclosing each letter in a box then removing common lines in a box
with open('advent12.txt','r') as file:
    garden = [line.strip() for line in file]

"""d={}
#area main plan but we need to seperate this since it is scattered
for i in text:
    if i in d:
        d[i]+=1
    else:
...
print(d)"""
#perimeter
#logic correct for perimeter and everything but plan scrapped
def calculate_regions(grid):
    ...
    def dfs(r, c, plant_type):
        ...
        while stack:
            x, y = stack.pop()
            area += 1
            # Count initial perimeter for the cell
            perimeter += 4
            ...
            for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                ...
                if 0 <= nx < rows and 0 <= ny < cols:
                    if grid[nx][ny] == plant_type:
                        # Shared side, decrement perimeter
                        perimeter -= 1
    ...
    for r in range(rows):
        for c in range(cols):
            if not visited[r][c]:
                ...
                results.append((plant_type, area, perimeter))

    return results

sum=0
regions = calculate_regions(garden)
for region in regions:
    plant, area, perimeter = region
    #print(f"Plant type {plant}: Area = {area}, Perimeter = {perimeter}")
    sum+=area*perimeter
print(sum)
with open("advent12.txt", "r") as file:
    garden_map = []
...
# Total: O(n)
#there is another method in chatgpt but it wasnt working proper;u
